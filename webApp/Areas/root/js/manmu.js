layui.config({ base: '/areas/root/js/' }).use(['form', 'table', 'glevent'], function () {
    ...
    form.on('submit(saveForm)', function (d) {
        var ds = d.field;
        var rDom = $('#result .layui-form-checked');
        if (rDom.length > 0) {
            var Arr = new Array();
            rDom.each(function () {
                Arr.push($(this).prev('input').attr('title'));
            });
            save({
                id: parseInt(ds.id),
                sid: parseInt(ds.sid),
                ...
                var i = 0, timer = setInterval(function () {
                    if (i < obj.length) {
                        save({
                            ...
                            C: obj[i].C,
                            D: obj[i].D,
                            E: obj[i].E !== undefined ? obj[i].E.length > 0 ? obj[i].E : null : null,
                            F: obj[i].F !== undefined ? obj[i].F.length > 0 ? obj[i].F : null : null,
                            explain: obj[i].exp,
                            result: obj[i].ans,
                            instate: true
                        }, function (res) {
    ...
    table.render({
        elem: '#tableDom', url: '/root/manmu/listdata',
        page: {
            hash: 'fenye',
            curr: location.hash.replace('#!fenye=', '') || 1
        }, limits: [10, 15],
        cols: [[
            ...
            { field: 'result', title: '答案', width: 110, unresize: true },
