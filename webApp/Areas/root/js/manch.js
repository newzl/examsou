layui.config({ base: '/areas/root/js/' }).use(['form', 'table', 'glevent'], function () {
    ...
    table.render({
        elem: '#tableDom', url: '/root/manch/listdata',
        page: {
            hash: 'fenye',
            curr: location.hash.replace('#!fenye=', '') || 1
        }, limits: [10, 15],
        cols: [[
            ...
            { field: 'result', title: '答案', width: 60, align: 'center', unresize: true },
    ...
    function initentity(did) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#opC').val(res.C);
                $('#opD').val(res.D);
                if (res.E !== null) $('#opE').val(res.E);
                if (res.explain !== null) $('#explain').val(res.explain);
                $('#result input').removeAttr('checked');
                $('#result input[value="' + res.result + '"]').attr('checked', true);
