...
namespace webApp.Areas.root.Controllers
{
    ...
    public class manJuController : Controller
    {
        //
        // GET: /root/manJu/
        ...
        {
            return View(BLL.root.subjuBLL.getEntity(id));
        }
        #endregion
        ...
        public void listdata()
        {
            try
            {
                ...
                {
                    table = "JudgementSubject k join subjectClass s on k.SubjectClassID=s.ID",
                    order = "k.createTime desc",
                    field = "k.id,s.name[sname],title,result,k.createTime[date],inputState[state],k.total,yes",
        ...
        [HttpPost]// /root/manju/save
        [ValidateInput(false)]
        public int save(Models.subju m)
        {
            m.creator = Utility.adminLogin.ID;
            return BLL.root.subjuBLL.save(m);
        }
        //修改入库状态
        [HttpPost]
        public int instate(int id, bool state)
        {
            BLL.root.subjuBLL.inState(id, state);
            return 1;
        }
        [HttpGet]
        public JsonResult getentity(int id)
        {
            return Json(BLL.root.subjuBLL.getEntity(id), JsonRequestBehavior.AllowGet);
        }
        #endregion
