...
namespace webApp.Areas.root.Controllers
{
    ...
    public class manChController : Controller
    {
        #region view
        ...
        {
            return View(BLL.root.subchBLL.getEntity(id));
        }
        #endregion
        ...
        public void listdata()
        {
            try
            {
                ...
                {
                    table = "ChooseSubject k join subjectClass s on k.SubjectClassID=s.ID",
                    order = "k.createTime desc",
                    field = "k.id,s.name[sname],title,result,A,B,C,D,E,k.createTime[date],inputState[state],k.total,yes",
        ...
        [HttpPost]// /root/manch/save
        [ValidateInput(false)]
        public int save(Models.subch m)
        {
            m.creator = Utility.adminLogin.ID;
            return BLL.root.subchBLL.save(m);
        }
        //修改入库状态
        [HttpPost]
        public int instate(int id, bool state)
        {
            BLL.root.subchBLL.inState(id, state);
            return 1;
        }
        [HttpGet]
        public JsonResult getentity(int id)
        {
            return Json(BLL.root.subchBLL.getEntity(id), JsonRequestBehavior.AllowGet);
        }
        #endregion
    }
}
